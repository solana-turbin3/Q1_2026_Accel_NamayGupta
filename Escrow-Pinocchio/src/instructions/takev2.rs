...
pub(crate) struct TakeV2Data {
    pub bump: u8,
}

pub fn process_take_v2_instruction(accounts: &[AccountView], data: &[u8]) -> ProgramResult {
    let [maker, taker, mint_a, mint_b, escrow_account, maker_ata_b, taker_ata_b, taker_ata_a, escrow_ata, _system_program, _token_program, _associated_token_program @ ..] =
        accounts
    else {
        return Err(ProgramError::NotEnoughAccountKeys);
    };

    let ix_data = wincode::deserialize::<TakeV2Data>(data)
    ...
    let (bump, amount_to_give, amount_to_receive) = {
        ...
        let escrow_pda = derive_address(
            &[b"escrow".as_ref(), maker.address().as_ref(), &[ix_data.bump]],
            None,
            &crate::ID.to_bytes(),
        );
