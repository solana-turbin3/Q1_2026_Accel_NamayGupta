...
mod tests {
    ...
    use crate::instructions::{CancelV2Data, MakeV2Data, TakeV2Data};
    use litesvm::LiteSVM;
    ...
    fn run_makev2(svm: &mut LiteSVM, maker: &Keypair) -> MakeState {
        let mint_a = CreateMint::new(svm, maker)
            .decimals(6)
            .authority(&maker.pubkey())
            .send()
        ...
        let make_v2_data = MakeV2Data {
            bump,
            amount_to_receive: amount_to_receive.to_le_bytes(),
            amount_to_give: amount_to_give.to_le_bytes(),
        };
        let serialized = serialize(&make_v2_data).expect("wincode serialize MakeV2Data");
        let make_data = [vec![3u8], serialized].concat();
        ...
        let message = Message::new(&[make_ix], Some(&maker.pubkey()));
        let tx = Transaction::new(&[maker], message, svm.latest_blockhash());
        svm.send_transaction(tx).expect("MakeV2 instruction failed");
    ...
    #[test]
    fn test_makev2_instruction() {
        let (mut svm, maker) = setup();
        let _state = run_makev2(&mut svm, &maker);
        println!("MakeV2 (wincode) OK");
    }

    #[test]
    fn test_takev2_instruction() {
        let (mut svm, maker) = setup();
        let state = run_makev2(&mut svm, &maker);
        ...
        let take_v2_data = TakeV2Data { bump: state.bump };
        let serialized = serialize(&take_v2_data).expect("wincode serialize TakeV2Data");
        let take_data = [vec![4u8], serialized].concat();
        ...
        let msg = Message::new(&[take_ix], Some(&taker.pubkey()));
        let tx = Transaction::new(&[&taker], msg, svm.latest_blockhash());
        svm.send_transaction(tx).expect("TakeV2 instruction failed");
        println!("TakeV2 (wincode) OK");
    }

    #[test]
    fn test_cancelv2_instruction() {
        let (mut svm, maker) = setup();
        let state = run_makev2(&mut svm, &maker);

        let cancel_v2_data = CancelV2Data { bump: state.bump };
        let serialized = serialize(&cancel_v2_data).expect("wincode serialize CancelV2Data");
        let cancel_data = [vec![5u8], serialized].concat();
        ...
        svm.send_transaction(tx)
            .expect("CancelV2 instruction failed");
        println!("CancelV2 (wincode) OK");
    }
}
