...
impl<'info> Stake<'info> {
    pub fn stake(&mut self, bumps: &StakeBumps) -> Result<()> {
        ...
        // Increment total_staked . Plugin added at create_collection;
        let fetched_collection_attrs = match fetch_plugin::<BaseCollectionV1, Attributes>(
            &self.collection.to_account_info(),
            PluginType::Attributes,
        ) {
            Err(_) => return Err(StakingError::InvalidAuthority.into()),
            Ok((_, attrs, _)) => attrs,
        };

        let mut attribute_list: Vec<Attribute> = Vec::new();
        for attribute in fetched_collection_attrs.attribute_list {
            if attribute.key == "total_staked" {
                let value = attribute
                    .value
                    .parse::<u64>()
                    .expect("Invalid total_staked value");
                attribute_list.push(Attribute {
                    key: "total_staked".to_string(),
                    value: value.checked_add(1).expect("Overflow error").to_string(),
                });
            } else {
                attribute_list.push(attribute);
            }
