...
pub fn process_initialize_instruction(accounts: &[AccountView], data: &[u8]) -> ProgramResult {
    let [maker, mint, fundraiser, _remaining_accounts @ ..] = accounts else {
    ...
    let amount_to_raise = u64::from_le_bytes(ix_data.amount_to_raise);
    let minimum_allowed = MIN_AMOUNT_TO_RAISE.pow(mint_state.decimals() as u32);
    assert!(amount_to_raise > minimum_allowed);
    ...
    unsafe {
        if fundraiser.owner() != &crate::ID {
            CreateAccount {
                from: maker,
                to: fundraiser,
                lamports: Rent::get()?.try_minimum_balance(Fundraiser::LEN)?, // can be done client side???
